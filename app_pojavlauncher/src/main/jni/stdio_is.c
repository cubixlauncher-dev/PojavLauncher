...
static bool splash_triggered = false;
...
static void *logger_thread() {
    ...
    while((rsize = read(pfd[0], buf, sizeof(buf)-1)) > 0) {
        ...
        if(!splash_triggered) {
            bool splash_detect = (strstr(buf, "version string") || strstr(buf, "Finished Initialization") || strstr(buf, "CubixChat successfully patched"));
            if(!splash_detect) continue;
            splash_triggered = true;
            if(splashListener != NULL) (*env)->CallVoidMethod(env, splashListener, logger_onSplashEvent);
        }
    }
...
Java_net_kdt_pojavlaunch_Logger_setSplashListener(JNIEnv *env, jclass clazz, jobject log_listener) {
    jobject splashListenerLocal = splashListener;
    if(log_listener == NULL) {
        splashListener = NULL;
    }else{
        if(splash_triggered) {
            splashListener = NULL;
            (*env)->CallVoidMethod(env, log_listener, logger_onSplashEvent);
        }else {
            splashListener = (*env)->NewGlobalRef(env, log_listener);
        }
    }
    if(splashListenerLocal != NULL) (*env)->DeleteGlobalRef(env, splashListenerLocal);
}